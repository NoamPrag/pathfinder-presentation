...
class NOrderBezier(PresentationScene):
    def write_title(self):
        title = Text("N-order Bèzier curve")
        title.shift(UP*3.5)
    ...
    def construct(self):
        ...
        bezier_points: list[complex] = [
            0, 1+1j, 1, -0.75j, 1.5, 3, 4+1.25j
        ]
        bezier = Bezier(points=bezier_points).translate(-5-1j).scale(2.5)
        ...
        s_eq_tex = MathTex("s =")
        s_eq_tex.shift(RIGHT*4.5)
        s_eq_tex.shift(DOWN*3)
        ...
        self.play(s_tracker.animate.set_value(0), Uncreate(bezier_f), run_time=3)
        self.wait(0.75)
        bezier_f = bezier.parametric_function().set_color(BLUE)
        self.play(s_tracker.animate.set_value(1), Create(bezier_f), run_time=3)
        self.end_fragment()
