...
class CentrifugalForce(PresentationScene):
    ...
    def get_tangent_circle(
        self, dist_tracker: ValueTracker, path: Path, path_derivative: Path
    ) -> Circle:
    ...
    def get_max_vel_graph(
        self, ax: Axes, dist_tracker: ValueTracker, path: Path
    ) -> VGroup:
        ...
        def get_graph_dots() -> VGroup:
            dist: float = (
                trajectory_length * dist_tracker.get_value() / scaled_path_length
            )
            graph = VGroup()
            prev_point_time = 0
            for i in range(0, len(trajectory)):
                ...
                if point.distance >= dist:
                    break  # plotting until dist_tracker's distance

                # limiting points byb time interval (graph's x-axis)
                if point.time - prev_point_time < time_interval:
                    continue
    ...
    def construct(self):
        ...
        bezier_fs: list[ParametricFunction] = [
            b.parametric_function() for b in path.beziers
        ]
        bezier_group = VGroup()
        ...
        dot = always_redraw(
            lambda: dot_from_complex(
                path.evaluate_by_distance(dist_tracker.get_value())
            ).set_color(BLUE)
        )
        ...
        vel_graph_no_centrifugal = get_vel_time_graph(trajectory, ax, dt=0.005)
        self.play(
            ShowIncreasingSubsets(vel_graph_no_centrifugal, run_time=2), run_time=2
        )
        ...
        limited_vel_graph = get_vel_time_graph(trajectory, ax, dt=0.001)
        ...
        trajectory = do_kinematics(trajectory)
        final_vel_graph = get_vel_time_graph(trajectory, ax, dt=0.001)
